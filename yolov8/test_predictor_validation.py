...
for scene in os.listdir('/WAVE/workarea/users/sli13/AIC23_MTPC/data/aic24/val') if not args.scene else [args.scene]:
    for camera in sorted(os.listdir(f'/WAVE/workarea/users/sli13/AIC23_MTPC/data/aic24/val/{scene}') if not args.reverse else os.listdir(f'/WAVE/workarea/users/sli13/AIC23_MTPC/data/aic24/val/{scene}'))[::-1]:
        # print(camera)
    # for camera in ['camera_0649']:
        #print(f'{camera=}')
        if '0649' not in camera:
            continue
