...
class Results(SimpleClass):
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        """
        frame = txt_file.split("~")[-1].split('.')[0]
        txt_file = f"{txt_file.split('~')[0]}.txt"
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                # line = (int(frame), c, *(d.xyxyxyxyn.view(-1) if is_obb else d.xywhn.view(-1)))
                line = (int(frame), *(d.xyxyxyxyn.view(-1) if is_obb else d.xywh.view(-1)))
                if masks:
                    seg = masks[j].xyn[0].copy().reshape(-1)  # reversed mask.xyn, (n,2) to (n*2)
                    line = (c, *seg)
                if kpts is not None:
                    # kpt = torch.cat((kpts[j].xyn, kpts[j].conf[..., None]), 2) if kpts[j].has_visible else kpts[j].xyn
                    # kpt = torch.cat((kpts[j].xy, kpts[j].conf[..., None]), 2) if kpts[j].has_visible else kpts[j].xy
                    kpt = kpts[j].xy
