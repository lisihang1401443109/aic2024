...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.save_dir = Path('/WAVE/workarea/users/sli13/AIC23_MTPC/yolov8/results')
        self.scene = None
        self.camera = None
    ...
    def write_results(self, idx, results, batch):
        ...
        # self.txt_path = self.save_dir + "/" + self.scene +'/' + self.camera + ("" if self.dataset.mode == "image" else f"~{frame}")
        self.txt_path = str(self.save_dir / self.scene / self.camera) + ("" if self.dataset.mode == "image" else f"~{frame}")
    ...
    def __call__(self, source=None, model=None, stream=False, *args, **kwargs):
        ...
        self.scene, self.camera = source.split('/')[-3:-1]
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        #     nl = len(list(Path(self.save_dir).glob("labels/*.txt")))  # number of labels
