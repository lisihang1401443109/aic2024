'''
Adapted from AIC23_Track1_UWIPL_ETRI/blob/8e23a69de57d2547b774399ae45792f83a2d17ef/STCRA/run_stcra.py


runs reassignment on the assigned tracks
...
import validation_cam_utils as cam_utils
...
def id_reassignment(tracks_list, dis_thr=200, conf_thr=0.7, try_second_nearest=True, verbose=False, scene = None):
    ...
    for cam_name, tracks in tracks_list.items():
        cam_info = cam_utils.CamInfo(scene, cam_name)
        for tid, track in tracks.items():
            for fid, location in track.items():
                # if multiple location for same tid in current fid
                if len(location) > 1:
                    if get_iou(location[0], location[1]) >= 0.7:
                        ...
                        max_loc_ind = np.argmax([loc[6] for loc in location])
                        tracks_list[cam_name][tid][fid] = [location[max_loc_ind]]
                    else:
                        # reassign_duplicate_ids.append((cam_name, tid, fid, location[0]))
                        # reassign_duplicate_ids.append((cam_name, tid, fid, location[1]))
                        for loc in location:
                            reassign_duplicate_ids.append((cam_name, tid, fid, loc))
                        tracks_list[cam_name][tid][fid] = []
                elif len(location):
                    # x, y, w, h = location[0][:4]
                    x2d, y2d = location[0][4], location[0][5]
                    # xworld, yworld = cam_utils.image2world([x2d, y2d], cam_utils.HOMO["S001"][cam_name]) # TODO: use our own honography matrix
                    xworld, yworld = cam_info.image2world(x2d, y2d)
    ...
    for tid, track in merge_tracks.items():
        for fid, loc_list in track.items():
            ...
            # to  prevent bug
            loc_list = np.array(loc_list, dtype=object)
            ...
            if len(loc_list) > 2:
                ...
                if len(delete_index) != len(coord):
                    ...
                    # logger.info(f'deleting {delete_index} from {loc_list}')
                    loc_list = np.delete(loc_list, delete_index, axis=0) 
            # check corner cases
            # ? suppressed because I dont know whats going on
            ...
            avg_loc = np.sum(np.array([_loc[1] for _loc in loc_list]), axis=0) / len(loc_list)
...
def parse_args():
    ...
    parser.add_argument('--input_folder_name', type=str, default='/WAVE/workarea/users/sli13/AIC23_MTPC/clustering/results/hungarian_cluster')
    parser.add_argument('--output_folder_name', type=str, default='/WAVE/workarea/users/sli13/AIC23_MTPC/clustering/results/reassignment')
...
def main():
    ...
    tracks_file_list = os.listdir("/WAVE/workarea/users/sli13/AIC23_MTPC/clustering/results/hungarian_cluster")
    tracks_file_list = [os.path.join(aic.cluster_path, file) for file in tracks_file_list if scene in file]
    print(tracks_file_list)
    
    logger.info(f'for scene {scene} ---------')
    logger.info(f'{tracks_file_list}')

    tracks_list = {
        _track_file.split('_')[-1][:-4]: load_tracking(_track_file, enable_feet=False) for _track_file in tracks_file_list
    }

    new_tracks_list, corrected_list, avg_tracks = id_reassignment(tracks_list, dis_thr=250, conf_thr=0.65, scene = scene, try_second_nearest=False)
    new_tracks_list, corrected_list, avg_tracks = id_reassignment(tracks_list, dis_thr=200, conf_thr=0.7, scene = scene, try_second_nearest=False)
    new_tracks_list, corrected_list, avg_tracks = id_reassignment(tracks_list, dis_thr=150, conf_thr=0.75, scene = scene, try_second_nearest=True)

    print('done reassignment for scene {}'.format(scene))
    ...
    if write_to_file:
        ...
        output_folder_name = aic[scene]['reassign']
        os.makedirs("{}".format(output_folder_name), exist_ok=True)
        for cam_name in tracks_list.keys():
            for tid, track in new_tracks_list[cam_name].items():
                ...
                output_filename = aic[scene, cam_name]['reassign']
                # logger.debug('writing to {}'.format(output_filename))
