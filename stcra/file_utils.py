...
def load_tracking(filename, debug_frame=0, enable_feet=False):
    ...
    with open(filename, 'r') as file:
        ...
        for line in file:
            ...
            frame_id, track_id = int(float(values[0])), int(values[1])
    ...
    for track_id, track in track.items():
        for frame_id, detection in track.items():
            if detection:
                ...
                # append results for xworld and yworld
                fx, fy = detection[0][4], detection[0][5]
            
                lines.append((frame_id, track_id, x, y, w, h, 1.0, -1, fx, fy))
